...
### 後続確認による整理（2026年9月24日）

次の項目は後続の検証で解消済みです。

- Tabキーによる入力欄、生成ボタン、生成結果、4つのコピーボタン、ページ内リンクへの移動は、2026年9月24日の共通テンプレート適用後の検証で確認済み
- 公開GitHub Pagesのルートページ、`js/diary-core.js`、`js/generators.js` がHTTP 200を返すことを確認済み

次の項目は、意図しないScrapboxページの作成や外部アプリケーションの起動を避けるため、この文書整理では実行していません。必要に応じて利用者環境で確認してください。

- 4つのコピーボタンからOSクリップボードへコピーし、貼り付けた内容が一致すること
- 生成されたPowerShellとBATを、意図した検証用Scrapboxプロジェクトを指定して実行すること
- 4つの正規actionから、意図したScrapboxページへ遷移すること
